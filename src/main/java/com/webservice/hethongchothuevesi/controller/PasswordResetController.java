...
public class PasswordResetController {
    ...
    public ResponseEntity<?> forgotPassword(@RequestBody ForgotPasswordRequest request) {
        try {
            ...
            return ResponseEntity.ok(new ApiResponse<>(1000, "Liên kết đặt lại mật khẩu đã được gửi tới email của bạn."));
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new ApiResponse<>(3000, "Đã xảy ra lỗi khi gửi email."));
        }
    }
    ...
    public ResponseEntity<?> resetPassword(@RequestBody ResetPasswordRequest request) {
        ...
        if (isReset) {
            ...
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(new ApiResponse<>(3000, "Token không hợp lệ hoặc đã hết hạn."));
        }
    }
}
